...
$\bigO{1}$ rather than in time proportional to $N$.
...
\begin{enumerate}
  ...
  \item A parallel characterisation of what that costs
        ...
        16-core host is hyperthread co-location at 35\% per worker rather than
        memory bandwidth. The mechanism is execution-port contention:
  ...
  \item A portable serialized state (\cref{sec:state}) that records a position
        rather than engine internals, refuses malformed input rather than
...
where the loop is bandwidth-bound and width buys nothing. The width is
therefore worth having at the tile size that matters.
...
rather than of the part: the same stepping on a whole two-socket host converts
...
That reading is not confirmed, and the 4-vCPU figure is reported rather than
...
determined by the partition rather than by the order in which threads happen to
...
This is verified rather than asserted: an 8191-block stream, chosen to straddle
every backend's interleaving width, is folded into a digest that is checked
against a fixed constant on every platform in continuous integration, and the
same terabyte of output was confirmed byte-identical across backends during the
statistical campaign of \cref{sec:stat}.
